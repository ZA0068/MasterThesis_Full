...
class Runner():
    ...
    def __acquire_data(self):
        for i in range(2):
            derivative = de
            __optimal_trajectory = read_data(f"rrt_trajectory_{self.__derivative.name}.csv")
            __drone_path_oiac = read_data(f"drone_path_{self.__derivative.name}_OIAC.csv")
            __drone_path_pid = read_data(f"drone_path_{self.__derivative.name}_PID.csv")
            __drone_force_and_torque_oiac = read_data(f"drone_force_and_torque_{self.__derivative.name}_OIAC.csv")
            __drone_force_and_torque_pid = read_data(f"drone_force_and_torque_{self.__derivative.name}_PID.csv")
            __durations = np.ones(self.__drone_path_oiac.shape[0]) * 0.05
    ...
    def plot_all_data_at_once(self, save_plot=False):
        self.__save_plots = save_plot
        self.__acquire_data()
        self.plot_3D_drone_path()
        self.plot_KD_values(self.get_drone_kd_values())
        self.plot_2D_drone_path()
