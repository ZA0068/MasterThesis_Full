...
from .model_reference_adaptive_controller import MRAC_PID
...
class DroneFeedbackController(Node):
    ...
    def _init_controller(self, controller_type: Controller):
        ...
        match controller_type:
            ...
            case Controller.MRAC:
                self._init_MRAC_controller()
    ...
    def _init_OIAC_controller(self):
        ...
        self.outer_loop = OIAC(50, 0.0, 10)
        self.outer_loop.set_saturation(-1, 1)
        self.inner_loop = OIAC(10, 0.0, 1)
    ...
    def _init_MRAC_controller(self):  # sourcery skip: extract-duplicate-method
        print("MRAC controller")
        self.throttle = MRAC_PID(2, 0, 0, 0.001)
        self.throttle.set_feedforward([5.45])
        self.throttle.set_dt(0.05)
        self.throttle.set_saturation(-1, 2)
        
        self.outer_loop = MRAC_PID(0.1, 0.0, 0, 0.00001)
        self.outer_loop.set_dt(0.05)
        self.outer_loop.set_saturation(-1, 1)
        
        self.inner_loop = MRAC_PID(0.2, 0.001, 1, 0.001)
        self.inner_loop.set_dt(0.05)
        self.inner_loop.set_saturation(-1, 1)
    ...
    def run_controller(self):
        match self.controller_type:
            ...
            case Controller.MRAC:
                self.run_MRAC_controller()
                
    def run_MRAC_controller(self):
        self._rp = self.outer_loop.run_MRAC_PID([self._position.x, self._position.y], self.__desired_trajectory[self._cnt, :2], [-0.1*self._velocity.x, -0.1*self._velocity.y])
        self._twist = self.inner_loop.run_MRAC_PID([self._orientation.x, self._orientation.y, self._orientation.z], [-self._rp[1], self._rp[0], 0.0])
        self._thrust.data = self.throttle.run_MRAC_PID(self._position.z, self.__desired_trajectory[self._cnt, 2], -2 * self._velocity.z).ravel()[0]
    ...
    def __trim_end(self):
        self._force_data = self._force_data[:self._max_timestep]
        self._error_data = self._error_data[:self._max_timestep]
        if self.controller_type == Controller.OIAC:
            self._K_values = self._K_values[:self._max_timestep]
            self._D_values = self._D_values[:self._max_timestep]

    def __remove_outliers(self):
        outlier_cnt = self.__count_for_outliers()
        self._pos_data = self._pos_data[outlier_cnt:]
        self._force_data = self._force_data[outlier_cnt:]
        self._error_data = self._error_data[outlier_cnt:]
        if self.controller_type == Controller.OIAC:
            self._K_values = self._K_values[outlier_cnt:]
            self._D_values = self._D_values[outlier_cnt:]

    def __count_for_outliers(self):
        return sum(
            self._pos_data[0][:] == self._pos_data[i][:]
            for i in range(len(self._pos_data))
        )
...
def main(args=None):
    ...
    try:
        drone_controller = DroneFeedbackController(controller_type=Controller.MRAC, trajectory_type=Derivative.SNAP)
