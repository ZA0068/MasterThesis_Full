...
class Runner():
    ...
    def plot_all_3D(self):
        ...
        plot_params = {
            ...
            "JERK_MRAC": {"color": "cyan", "linestyle": "dotted", "linewidth": 6},
            "SNAP_OIAC": {"color": "darkgreen", "linestyle": "--", "linewidth": 8},
            "SNAP_MRAC": {"color": "navy", "linestyle": "dotted", "linewidth": 6}
        }
        for index, gaussian in enumerate(gaussian_values):
            ...
            for key_suffix, params in plot_params.items():
                ...
                df = self.__optimal_trajectory.get(key, pd.DataFrame()) if 'OIAC' not in key_suffix and 'MRAC' not in key_suffix else self.__drone_trajectory.get(key, pd.DataFrame())
    ...
    def plot_2D_drone_path(self):
        ...
        self.__plotter.set_trajectory(self.__drone_path_mrac)
        self.__plotter.plot_2d_trajectory(label = "rrt-star drone path MRAC")
    ...
    def plot_drone_force_and_torque(self):
        ...
        oiac_force_n_torque, mrac_force_n_torque = equalize_data_length(self.__drone_force_and_torque_oiac, self.__drone_force_and_torque_mrac)
        self._rename_plot_drone_force_and_torque(
            oiac_force_n_torque, 3, mrac_force_n_torque, ' trajectory: Throttle'
        )
        self.__plotter.plot_KD_values(index=range(2), ylabel="Force [N]")
        self._rename_plot_drone_force_and_torque(
            oiac_force_n_torque, 0, mrac_force_n_torque, ' moments: Roll'
        )
        self.__plotter.plot_KD_values(index=range(2, 4), ylabel="Torque [Nm]")
        self._rename_plot_drone_force_and_torque(
            oiac_force_n_torque, 1, mrac_force_n_torque, ' moments: Pitch'
        )
        self.__plotter.plot_KD_values(index=range(4, 6), ylabel="Torque [Nm]")
        self._rename_plot_drone_force_and_torque(
            oiac_force_n_torque, 2, mrac_force_n_torque, ' moments: Yaw'
        )
        self.__plotter.plot_KD_values(index=range(6, 8), ylabel="Torque [Nm]")
        self.__plotter.reset()

    def _rename_plot_drone_force_and_torque(self, oiac_force_n_torque, arg1, mrac_force_n_torque, arg3):
        self.__plotter.add_other_data(oiac_force_n_torque[:, arg1], "OIAC")
        self.__plotter.add_other_data(mrac_force_n_torque[:, arg1], "MRAC")
    ...
    def build_optimal_trajectory(self, derivative: Derivative):
        ...
        save_data("durations.csv", self.__durations)
    ...
    def plot_drone_force_and_torque_all(self):
        ...
        combinations = [('JERK', 'OIAC'), ('JERK', 'MRAC'), ('SNAP', 'OIAC'), ('SNAP', 'MRAC')]
